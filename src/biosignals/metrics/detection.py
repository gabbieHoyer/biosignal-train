...
@torch.no_grad()
def f1_from_counts_tensor(tp: int, fp: int, fn: int, device: torch.device | None = None) -> torch.Tensor:
    denom = (2 * tp + fp + fn)
...
def event_f1_1d(
    ...
    *,
    ...
    pred_events = extract_events_from_probs(probs_1d, valid_mask, threshold=threshold, min_len=min_len)
