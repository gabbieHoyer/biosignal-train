...
class TCNSegmenter(nn.Module):
    ...
    def __init__(
        ...
        self.encoder = TCNEncoder1D(in_channels=int(in_channels), width=int(width), depth=int(depth))
        self.head = SegmentationHead(in_dim=int(self.encoder.out_channels), num_classes=int(num_classes))
