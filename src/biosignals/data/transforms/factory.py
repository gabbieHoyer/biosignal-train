...
def build_transform(spec: Any) -> Transform:
    ...
    if isinstance(spec, (list, tuple)):
        items = [_coerce_item(x, context=f"python {type(spec).__name__}[{i}]") for i, x in enumerate(spec)]
    ...
    if OmegaConf.is_config(spec):
        ...
        if isinstance(spec, DictConfig) and ("train" in spec and "val" in spec) and ("_target_" not in spec) and ("transforms" not in spec):
            raise TypeError(
        ...
        if isinstance(spec, DictConfig):
            ...
            if "transforms" in spec and "_target_" not in spec:
                ...
                items = [_coerce_item(x, context=f"DictConfig.transforms[{i}]") for i, x in enumerate(list(items_cfg))]
            ...
            if "_target_" in spec:
                ...
                if target.endswith(".Compose") or target.endswith("Compose"):
                    ...
                    items = [_coerce_item(x, context=f"Compose.transforms[{i}]") for i, x in enumerate(list(items_cfg))]
...
def _coerce_item(x: Any, *, context: str) -> Transform:
    ...
    if OmegaConf.is_config(x):
        if isinstance(x, DictConfig) and "_target_" not in x:
            raise TypeError(
                f"{context}: transform item is a DictConfig without _target_. "
                f"Value={x}"
            )
        ...
        if not callable(inst):
            raise TypeError(f"{context}: instantiated item is not callable. type={type(inst)} value={inst}")
        return inst  # type: ignore[return-value]

    # Anything else is invalid
    raise TypeError(f"{context}: transform item is neither callable nor OmegaConf config. type={type(x)} value={x}")
...
def _ensure_transform_callable(inst: Any, *, context: str) -> Transform:
    ...
    raise TypeError(f"{context}: instantiated object is not callable. type={type(inst)} value={inst}")
